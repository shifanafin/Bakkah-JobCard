...
> = {
  ...
  in_progress: {
    ...
    bg: "bg-brand-50 dark:bg-brand/10",
    border: "border-brand-200 dark:border-brand/20",
    dot: "bg-brand",
  },
  qc_check: {
    label: "Quality Check",
...
const ANNOUNCEMENT_STYLE: Record<string, string> = {
  promo:
    "bg-gradient-to-r from-brand to-golden text-black border-golden-300",
    ...
    <div className="flex gap-1.5">
      {[1, 2, 3, 4, 5].map((i) => (
        ...
        >
          <Star
            className={cn(
              "h-8 w-8 transition-colors drop-shadow-sm",
              i <= (hovered || value)
...
export default function TrackPage() {
  ...
  async function runSearch(q: string) {
    ...
    setFbRating(0);
    setFbComment("");
    const queryType = q.toUpperCase().startsWith("JC-")
      ? "job_number"
      : "phone";
    try {
      const sb = createClient();
  ...
  const statusCfg = result
    ? (STATUS_CONFIG[result.status] ?? STATUS_CONFIG.received)
    : null;
      ...
      <main className="mx-auto max-w-2xl px-4 py-6 sm:py-10">
          ...
          <div className="mb-5 space-y-2">
            {visibleAnnouncements.map((a) => (
              <div
                key={a.id}
                className={cn(
                  "relative flex items-start gap-3 rounded-2xl border px-4 py-3.5 shadow-sm",
        ...
        <form onSubmit={handleSearch} className="card-elevated mb-5">
          ...
          <p className="mt-2.5 text-xs text-gray-400 dark:text-white/30 leading-relaxed">
            {tr.search.hint}{" "}
            <span className="font-mono font-bold text-brand">JC-</span>{" "}
            {tr.search.hint2}
          </p>
        </form>
        ...
        {result && statusCfg && (
          <div className="space-y-4">
            ...
            <div className="card-elevated overflow-hidden">
              ...
              <div
                className={cn(
                  "h-1.5 -mx-5 -mt-5 mb-5 rounded-t-2xl",
                  statusCfg.dot,
                )}
              />
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-white/50">
                    {result.vehicle?.make} {result.vehicle?.model} &middot;{" "}
                ...
                <div className="mb-5">
                  ...
                  <div className="hidden sm:block">
                    <div className="relative">
                      ...
                      {/* Progress bar fill */}
                      <div
                        className="absolute top-4 left-4 h-0.5 bg-gradient-to-r from-brand to-golden status-progress"
                        style={{
                          width: `calc(${(curStep / (STEPS.length - 1)) * 100}% - 2rem)`,
                        }}
                      />
                        ...
                        {STEPS.map((step, i) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={cn(
                                  "flex h-8 w-8 items-center justify-center rounded-full border-2 text-xs font-bold transition-all duration-500",
                                    ...
                                    : active
                                      ? "border-brand bg-brand text-black shadow-[0_0_16px_rgba(107,122,40,0.4)]"
                              ...
                              >
                                {(tr.status as Record<string, string>)[step] ??
                                  step}
                              </span>
                            </div>
                          );
                  ...
                  <div className="sm:hidden">
                    ...
                    >
                      <div
                        className={cn(
                          "flex h-10 w-10 items-center justify-center rounded-full font-black text-sm shrink-0",
                          statusCfg.dot === "bg-brand"
                            ? "bg-brand text-black"
                            : `${statusCfg.dot} text-white`,
                        )}
                      >
                        {curStep + 1}
                      </div>
                      <div>
                        <p className={cn("text-sm font-bold", statusCfg.color)}>
                          {(tr.status as Record<string, string>)[
                            result.status
                          ] ?? result.status}
                        </p>
              ...
              <div className="card">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-mono font-black text-gray-900 dark:text-white tracking-wider text-base">
                      {result.vehicle?.plate_number}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
              ...
              <div className="card">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-700 dark:text-white/70 font-semibold">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <a
                href={`https://wa.me/971545886999?text=Hi+Bakkah%2C+I+want+to+follow+up+on+job+${encodeURIComponent(result.job_number)}`}
                target="_blank"
                rel="noopener noreferrer"
              ...
              {reviews.slice(0, 6).map((r) => (
                <div key={r.id} className="card-hover space-y-3">
                  ...
                  <div className="flex items-center gap-2.5 pt-1">
                    ...
                    <div>
                      <p className="text-xs font-bold text-gray-700 dark:text-white/60">
                        {r.customer_name}
                      </p>
        ...
        <div className="mt-12 rounded-2xl border border-gray-100 dark:border-white/[0.06] bg-white dark:bg-surface-800 p-5 text-center space-y-3">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="font-display tracking-[0.2em] text-gray-700 dark:text-white/70 text-sm">
              BAKKAH
            </span>
          </div>
        ...
        <div className="mt-4 flex items-center justify-center gap-4 text-xs text-gray-400 dark:text-white/25">
          <Link
            href="/"
            className="hover:text-gray-600 dark:hover:text-white/50 transition-colors flex items-center gap-1"
          >
