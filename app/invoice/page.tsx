...
import {
  Search, Loader2, Car, FileText, Sun, Moon, MessageCircle,
  ArrowLeft, Check, Camera, CreditCard, Hash,
} from 'lucide-react'
    ...
    try {
      ...
      if (isJobNumber) {
        const { data, error: err } = await sb
          .from('job_cards').select('id').ilike('job_number', q).limit(1).single()
        if (err || !data) { setError('No invoice found for that job number. Please check and try again.'); return }
        router.push(`/invoice/${data.id}`)
      } else {
        const { data: customers } = await sb
          .from('customers').select('id').ilike('phone', `%${q.replace(/\s+/g, '')}%`).limit(5)
        if (!customers || customers.length === 0) { setError('No records found for that phone number.'); return }
        const ids = customers.map((c: { id: string }) => c.id)
        const { data: job, error: jobErr } = await sb
          .from('job_cards').select('id').in('customer_id', ids).order('created_at', { ascending: false }).limit(1).single()
        if (jobErr || !job) { setError('No invoice found for that phone number.'); return }
      ...
      <main className="mx-auto max-w-2xl px-4 py-8 sm:py-12">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-base text-gray-500 dark:text-white/50 max-w-sm mx-auto leading-relaxed">
            Enter your job number or phone number to instantly access your full service invoice.
          </p>
        </div>

        {/* Search form */}
        <div className="card-elevated mb-6">
          <p className="text-sm font-semibold text-gray-700 dark:text-white/70 mb-3">Find your invoice</p>
          <form onSubmit={handleSearch}>
            ...
            <p className="mt-2.5 text-xs text-gray-400 dark:text-white/30">
              Use your <span className="font-mono font-bold text-brand">JC-XXXX-XXXX</span> job number or the phone number used at booking.
            </p>
          </form>
        </div>
        ...
        <div className="card mb-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-bold text-gray-900 dark:text-white">What&apos;s in your invoice?</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {WHATS_INSIDE.map(({ icon: Icon, label }) => (
              <div key={label} className="flex items-center gap-2.5 rounded-xl border border-gray-100 dark:border-white/[0.05] bg-gray-50/70 dark:bg-white/[0.02] px-3.5 py-2.5">
        ...
        <div className="card mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-bold text-gray-900 dark:text-white">How to find your job number</h2>
          </div>
          <ol className="space-y-3">
            {[
              { step: 1, text: 'Check the SMS or WhatsApp message you received when your vehicle was booked in.' },
              { step: 2, text: 'It starts with JC- followed by the year and a number, e.g. JC-2026-0042.' },
              { step: 3, text: 'You can also use the mobile number you gave us at reception.' },
            ].map(({ step, text }) => (
              <li key={step} className="flex items-start gap-3 text-sm text-gray-500 dark:text-white/50">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand/10 text-[10px] font-black text-brand mt-0.5">{step}</span>
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-white/[0.07] bg-white dark:bg-surface-800 p-5 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900 dark:text-white">Track your vehicle status</p>
              <p className="text-xs text-gray-500 dark:text-white/40 mt-0.5">See real-time service progress for your car</p>
            </div>
          </div>
        ...
        <div className="text-center">
          <Link href="/" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-gray-600 dark:text-white/30 dark:hover:text-white/60 transition-colors">
        ...
        <div className="mt-10 rounded-2xl border border-gray-100 dark:border-white/[0.06] bg-white dark:bg-surface-800 p-4 text-center space-y-2 shadow-sm">
          <p className="text-xs text-gray-400 dark:text-white/30">Need help? We&apos;re available 7 days a week</p>
          <a href="https://wa.me/971589397610" target="_blank" rel="noopener noreferrer"
