...
import { ChevronLeft, Lightbulb, Plus, Loader2, Check, X, Clock, Edit2, Trash2 } from 'lucide-react'
    ...
    try {
      const res = await fetch('/api/my/feedback', {
        ...
        body: JSON.stringify(editingId
          ...
          : { type: 'suggestion', subject: form.subject, message: form.message }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      toast.success(editingId ? 'Suggestion updated' : 'Suggestion submitted! Thank you.')
      setShowForm(false)
      setEditingId(null)
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-surface-900">
        ...
        {showForm && (
          <div className="card space-y-4">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="text-base font-bold text-gray-900 dark:text-white">{editingId ? 'Edit Suggestion' : 'Share Your Idea'}</h3>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={submitting} className="btn-primary w-full">
                {submitting
                  ...
                  : <><Check className="h-4 w-4" /> {editingId ? 'Save Changes' : 'Submit Suggestion'}</>}
              </button>
            </form>
          </div>
        )}
      ...
      <ConfirmDialog
        ...
        message="This will permanently delete your suggestion."
        confirmLabel="Delete"
        variant="danger"
        loading={deleting}
