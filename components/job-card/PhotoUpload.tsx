...
export default function PhotoUpload({ jobCardId, photos, onPhotosChange, readOnly }: {
  jobCardId: string; photos: JobCardPhoto[]; onPhotosChange: (p: JobCardPhoto[]) => void; readOnly?: boolean
}) {
  ...
  return (
    <div className="card space-y-5">
      ...
      {/* Upload controls — hidden when locked */}
      {!readOnly && (
        <>
      ...
      {photos.length === 0 && (
        <p className="text-center text-sm text-white/20 py-4">{readOnly ? 'No photos on record.' : 'No photos yet. Upload vehicle photos above.'}</p>
      )}
